        ...
        <div className="space-y-4">
          {cards.map((c)=>(
            <div key={c.type} className="card p-4 flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl ${c.bg}`}>{c.emoji}</div>
              <div className="flex-1">
                <p className="font-bold">{c.label}</p>
                <p className="text-sm text-gray-500">{c.count} clases</p>
              </div>
              <button onClick={()=> nav(`/classes/${c.type}`)} className="btn btn-primary">Ver</button>
            </div>
          ))}
        </div>
      </div>
