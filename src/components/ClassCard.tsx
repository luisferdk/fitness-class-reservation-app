  ...
  return (
    <div className={`card p-4 flex items-center gap-4 ${disabled?'opacity-60':''}`}>
      <div className="flex-1">
        <p className={`font-extrabold ${disabled?'text-gray-400':'text-black'}`}>{formatTime(item.time)}</p>
        <p className={`text-sm ${disabled?'text-gray-400':'text-gray-700'}`}>Coach: {item.coach}</p>
        <p className={`${item.spots>0?'text-green-600':'text-rose-500'} text-sm font-medium mt-1`}>
